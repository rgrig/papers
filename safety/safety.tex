...
\grammar{
  Program& Class\*\cr
  Class& id \b\{ Member\* \b\}\cr
  Member& TypeAndId& field\cr
  Member& TypeAndId \b( FormalArgs \b) body& method\cr
  Member& TypeAndId \b( FormalArgs \b)& abstract method\cr
  TypeAndId& id id\cr
  Body& \b\{ Statement\* \b\}\cr
}

\noindent There are five basic statements and two composed statements.

\grammar{
  Statement& \b{return} Var\cr
  Statement& Var \b{:=} Var& stack assignment\cr
  Statement& Var \b{:=} Var \b. id& heap assignment\cr
  Statement& Var \b{:=} Var \b. id Args& method call\cr
  Statement& Var \b{:=} \b{new} id& memory allocation\cr
  Statement& \b{while} Body\? Condition Body\?& loop\cr
  Statement& \b{if} Condition Body& branching\cr
}
