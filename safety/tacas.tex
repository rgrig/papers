...
\paragraph{TOPL execution automata.}
...
$\set{Vertex}$ is the set of vertices mentioned as endpoints of the arcs and $\set{Arc}$ is the set of labeled arcs mentioned in transitions. The automaton has two special vertices, {\tt start} and {\tt error}, the initial and accepting vertex respectively. 
...
\]
Moreover we assume a countable set \set{Event} of events with known arity. Each event~$e$ contains an array of values.
...
Give a TOPL property $(\set{Vertex},\set{Arc})$ and a set of program traces $\set{Trace}$, a {\em TOPL execution automaton} is a tuple 
...
Since different outgoing arcs for a vertex can have different depths, 
therefore different target states of execution transitions may receive different events.
...
In order to define the transition relation we need to define two auxiliary concepts. 
...
A(\sigma,e\cdot \tau,(g,a)\cdot l) & = & \left\{\begin{array}{ll}
     A(\sigma',\tau,l) & \mbox{if $|\tau|=|l|>0$ and $A(\sigma,e,(g,a))=\sigma'$}
     \\
     \bot & \mbox{otherwise}
     \end{array}
     \right.
...
$Enabled$ testing if there exists an enabled transition starting from a vertex, given a store and a trace of events:
\begin{eqnarray*}
...
\[
\begin{array}{l}
\infer[x_1 \to x_2: l \mbox{ and } |l|=|\tau_1| ]{ (x_1,\sigma_1,\tau_1 \circ \tau_2 ) \  \to \  (x_2, \sigma_2, \tau_2)}{A(\sigma_1,\tau_1,l)=\sigma_2}
\\[2ex]
\infer[\neg Enabled(x_1,\sigma_1,\tau_1) ]{ (x_1,\sigma_1,\tau_1) \  \to \  (x_1, \sigma_1, \tau_2)}{ \tau_1=e \cdot \tau_2}
\end{array}
\]
\dinocomment{********** UP TO HERE *********}
