...
\end{align*}
If $\cal A$~has $m$~registers, $|\delta|$~transitions, $|Q|$~states, and works over $n$-tuples, then ${\cal A}'$~has $2m+1$ registers, $|\delta'|=O(n(2m)^{2m}|\delta|)$ transitions, and  $O((2m)^m|Q|+|\delta'|)$~states.
\end{lemma}
\begin{proof}
