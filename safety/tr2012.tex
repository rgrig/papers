...
\[ (q_\rho, [(g_0,a_0),\ldots,(g_{d'-1},a_{d'-1})], q'_\rho)
  \qquad\text{with $d'<d$} \]\rlp{$d'-1\mapsto d'$?}
...
\]
where $a_p;a'_{d'-1}$ is an action that simulates applying first $a_p$ and then $a'_{d'-1}$ and $r'_p$ is the resulting distribution function.


If the state $q=(q_\rho,d,k,r)$ has no outgoing transitions at this point, it is because none of the outgoing transitions of~$q_\rho$ have guards that all hold.
...
If the state does have outgoing transitions, we must also simulate a skip transition, but only to be taken in case none of the other transitions are.
This is done using the same construction employed in the proof of Lemma \ref{lemma:topl-to-rtopl}.
