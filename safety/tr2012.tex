...
The language $L\subseteq C^*$ could be defined, for example, using a finite automaton, or a context free grammar.
If $L$~is defined using a finite automaton, then ${\cal L}_\sigma$ is expressible by TOPL automata.
...
\rg{TODO: Explain why for negative results we must be more careful with how we define `expressivity'.}
...
Conversely, there are TOPL languages that are not expressible by slicing, no matter how $L\subseteq C^*$ is chosen.
Consider the language
\begin{align*}
\{\,(1,v_1)\ldots(1,v_n)\mid\text{$v_i=v_1$ for some $i>1$}\,\}
\end{align*}
which is related to that of \autoref{ex:slicing_lang}.
The projections $\pi_c(w\searrow\theta)$ are simply strings $11\ldots1$, and there is no way to decide if the first letter was retained or not by slicing.
\rg{This is not fair to slicing.}
