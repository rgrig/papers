...
There is one such path for each $r_0\in[m]\to[2m]$.
...
Suppose $g$ contains the conjuncts ${\sf eq}\,i=j$ for $i\in I$, the conjuncts ${\sf neq}\,i\ne j$ for $i\in I'$, and no other conjunct that refers to component~$j$.
If $|r_0(I)|\ge2$ or $|r_0(I)\cap r_0(I')|\ge1$, then the path from $(q_0,r_0)$ is discarded from the construction of~${\cal A}'$.
If $r_0(I)=\{i\}$, then $g_j$ is ${\sf eq}\,i$.
If $r_0(I)=\emptyset$, then $g_j$ is {\sf fresh}.

\rg{Fix what follows.}

\paragraph{Case ${\sf eq}\,i'$.}
In this case the value of the current letter is already in register~$i'$.
The action $a_j$ is {\sf nop}.
The function $r_j$ is computed from~$r_{j-1}$:
\begin{align*}
r_j(i) = \begin{cases}
  i' &\text{if ${\sf set}\,i:=j$ occurs in $a$} \\
  r_{j-1}(i) &\text{otherwise}
\end{cases}
\end{align*}

\paragraph{Case ${\sf fresh}$.}
TODO.

At this point the guards are 
...
%\underbrace{\ell_\bot\ldots\ell_\bot}_{\text{$d+1$~times}}
\in{\cal L}({\cal A})\]
...
We must cater for two situtaions: Either more letters are arriving and we should simulate storing them in the queue and possibly make a state transition or no more letters are arriving and we should simulate emptying the queue to see if we end up in an accepting state. Crucially, enough information is available to statically simulate receiving all the letters in the queue.

To treat the first situation, we add transitions in one of two ways described below. The first description goes into detail about why it is possible to simulate ${\cal A}_\rho$ from a state in ${\cal A}$.
...
\smallskip
Finally, for each state, we statically compute if emptying the queue would lead to a final state. If it would then that state is marked as final.
(A special case of this is the states with $h=0$.)
\end{proof}
