...
% >>>
% >>>
\section{Comparison with Trace Slicing} \label{sec:slicing} % <<<
...
In this section the alphabet is
\begin{align*}
\Sigma = C \times V_\bot^{n-1}
\end{align*}
where $C$ is a finite subset of~$V$.
Each letter~$\ell\in\Sigma$ is a pair~$(c,\theta)$ of a \emph{constant}~$c\in C$ and a \emph{binding}~$\theta\in V_\bot^{n-1}$.
Alternatively, a letter is an $n$-tuple $(v_1,\ldots,v_n)$ with $v_1\in C$ and $(v_2,\ldots,v_n)\in V_\bot^{n-1}$.
...
The slice~$w\searrow\theta$ retains the constants that are paired with bindings~$\sqsubseteq\theta$.
For words of length~$1$ slicing gives an element of $C^0\cup C^1$.
\begin{align*}
(c',\theta')\searrow\theta=\begin{cases}
  c' & \text{if $\theta'\sqsubseteq\theta$} \\
  \epsilon & \text{otherwise}
\end{cases}
\end{align*}
Slicing is extended pointwise to words of arbitrary length.
\begin{align*}
  \searrow &\in (C\times V_\bot^{n-1})^*\times V_\bot^n \to C^*
\end{align*}
Every slice is a word over the finite alphabet~$C^*$.

\begin{example}
Let $V$ be the set~$\{1,2,\ldots\,\}$ of positive integers, let $C=[4]$, and let $n=3$.
\begin{align*}
(1,\bot,\bot)(2,1,\bot)(3,\bot,1)(4,1,1)\searrow(1,1) &= 1\,2\,3\,4 \\
(1,\bot,\bot)(2,1,\bot)(3,\bot,1)(4,1,1)\searrow(\bot,1) &= 1\,3
\end{align*}
\eoe
\end{example}

Each language $L \subseteq C^*$ determines a language ${\cal L}_\sigma(L) \subseteq \Sigma^*$:
\begin{align*}
w \in {\cal L}_\sigma(L)
  \qquad\iff\qquad
\text{$w\searrow\theta \in L$ for some binding~$\theta$}
\end{align*}
...
\end{align*}
On the other hand, the related language
\begin{align*}
\{\,(1,v_1)\ldots(1,v_n)\mid\text{$v_i=v_1$ for some $i>1$}\,\}
\end{align*}
is not definable by traces, although it is definable by TOPL automata.
\eoe
\end{example}

If the language~$L$ is the regular language~${\cal L}({\cal A})$ of some finite automaton~$\cal A$, then ${\cal L}_\sigma(L)$ is recognizable by TOPL automata.
In other words, TOPL automata are at least as expressive as slicing that uses finite automata as the underlying mechanism.
...
\end{lemma}
\begin{proof}
\rg{Use the states $Q\times([n]\to2)$, to keep track of which components of~$\theta$ are bound.
Keep $\theta$ in registers.}
\end{proof}

In light of \autoref{lemma:slicing-to-topl} and \autoref{ex:slicing} it is tempting to conclude that TOPL automata are strictly more expressive than slicing with finite automata.
The difference exhibited by \autoref{ex:slicing}, however, is technical rather than substantial.
\rg{We need to exhibit a class of TOPL languages for which there is no quick reduction of membership to membership in some slicing language.}
