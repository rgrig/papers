...
Runtime verification with automata boils down to solving the membership problem for some one-way automaton.
The membership problem asks whether a given word~$w$ belongs to the language of an automaton~${\cal A}$.
Each arrow in \autoref{fig:concepts} represents a reduction of the membership problem: an algorithm that transforms an instance of the problem for one model of automata into an instance for another model of automata.
The word undergoes only trivial transformations to overcome technicalities.
However, in some cases the output automaton is significantly bigger than the input automaton.
...
The alphabet~$\Sigma=V^n$ is an $n$-tuple of values from a possibly infinite set $V$.
...
we write $(\ell,\sigma)\models g$ to denote that letter $\ell\in\Sigma$ and store $\sigma\in S$ satisfy the guard~$g$, and we denote the set of guards by~$G$.
...
\begin{definition}
The \emph{configuration graph} of an automaton consists of exactly those configuration transitions \[ (q_1,\sigma_1) \stackrel{\ell}{\to} (q_2,\sigma_2) \] for which there exists an automaton transition $(q_1,(g,a),q_2)\in\delta$ such that $(\ell,\sigma_1)\models g$ and $(\sigma_1,\sigma_2)\in a(\ell)$.
\end{definition}
...
\[ \bigl((v_1,\ldots,v_m),(v'_1,\ldots,v'_m)\bigr)\in
  ({\sf set}\,i:=j)\bigl((w_1,\ldots,w_n)\bigr) \]
if and only if $v'_k=v_k$ for all~$k$, except that $v'_i=w_j$.
...
\begin{align*}
G_{\rm RA} &::= {\sf fresh}\\
G_{\rm RA} &::= {\sf equals}\,i
\end{align*}
They are interpreted as follows:
\begin{align*}
\bigl(w,(v_1,\ldots,v_m)\bigr)&\models{\sf fresh}
  &&\text{when $w\ne v_k$ for all $k$} \\
\bigl(w,(v_1,\ldots,v_m)\bigr)&\models{\sf equals}\,i
  &&\text{when $w=v_i$}
\end{align*}
...
\begin{definition}
A \emph{register automaton} with $m$~registers is an automaton over the alphabet $\Sigma=V$ whose labels have the shape $({\sf fresh},\;{\sf set}\,i)$ or the shape $({\sf equals}\,i,\; {\sf nop})$, where $i\in\{1,\ldots,m\}$.
\label{def:ra}
\end{definition}

There are several variants of definitions for register automata in the literature.
It is even the case that the complexity of the emptiness problem depends on the exact definition one chooses.
Our definition corresponds directly to what Neven et al~\cite{dblp:conf/mfcs/nevensv01} call 1N-RA (for `one-way non-deterministic register automata').
The following theorem summarizes some known results that apply to the definition we chose.

\begin{theorem}
The following properties of register automata are known:
\begin{enumerate}
\item The emptiness problem is NP-complete~\cite{dblp:journals/tcs/sakamotoi00}.
\item The membership problem is NL-complete~\cite{dblp:conf/mfcs/nevensv01}.
\item The containment, equivalence, and universality problems are decidable for \emph{two} registers.~\cite{dblp:conf/focs/kaminskif90}
...
All of these result carry over, perhaps slightly modified, to TOPL automata.
