...
\hypersetup{
  pdftitle={Runtime Verification with Register Automata},
  pdfauthor={Radu Grigore and Dino Distefano and Rasmus Lechedahl Petersen}}

\title{Runtime Verification with Register Automata}
...
TOPL is an extension of register automata~\cite{dblp:conf/focs/kaminskif90}.
...
If the automaton if clear from the context we omit the subscript.
...
It is easy, however, to transform a TOPL automaton with an arbitrary $n$ into an equivalent TOPL automaton with $n=1$.
...
It is, therefore, enough to transform TOPL automata with $n=1$ into register automata.

One difficulty presents itself by comparing the guards available in $G_{\rm TOPL}$ and $G_{\rm RA}$:
The TOPL automata guard ${\sf neq}\,i$ is more precise than its register automata equivalent, namely ${\sf fresh}$.
More generally, the labels of register automata do not allow storing a value twice:
Transition that contain an assignment must have the guard ${\sf fresh}$.
The key idea is of the encoding the following.
\emquote{The set of registers holding value~$v$ in a configuration of the TOPL automaton are represented by one register holding value~$v$ in a configuration of the equivalent register automaton.}
Let us develop the idea in some detail.
Consider a TOPL automaton with $m$~registers and states~$Q$.
The equivalent register automaton we build has $m$~registers and states $Q\times{\cal P}([m])^m$.
The register automaton configuration
\[\bigl((q,(I_1,\ldots,I_m)),\;(u_1,\ldots,u_m)\bigr)\]
relates to the TOPL automaton configuration
\[(q,(v_1,\ldots,v_m))\]
as follows:
\[u_i=v_j \iff j\in I_i\]
Two sets $I_i$~and~$I_j$ are disjoint, for $i\ne j$.
If $I_i=\emptyset$, then $u_i$ is different from all the values held in the registers of the TOPL automaton, but is otherwise unconstrained.
Each state of the TOPL automaton is encoded with $2^{m^2}$~states in the register automaton.

Each transition $(q_1,(g,a),q_2)$ of the TOPL automaton is encoded into several transitions of the register automaton that have the shape
\[\Bigl(\bigl(q_1,(I_1,\ldots,I_m)\bigr), (g',a'),\bigl(q_2,(I'_1,\ldots,I'_m)\bigr)\Bigr)\]
For each $g$,~$a$, $I_1$,~$I_2$ \dots,~$I_m$, we will specify several $g'$,~$a'$, $I'_1$,~$I'_2$ \dots,~$I'_m$.
After the transformation to unary alphabets there are two types of labels, corresponding to the first $n$~transitions encoding guards and, respectively, the last $n$~transitions encoding actions:
\begin{itemize}
\item $({\sf eq}\,i_1\mathrel{\sf and}{\sf eq}\,i_2\ldots
  \mathrel{\sf and}{\sf neq}\,i'_1\mathrel{\sf and}{\sf neq}\,i'_2\ldots,\;
  {\sf nop})$ which we abbreviate as
  $({\sf eq}\,I\mathrel{\sf and}{\sf neq}\,I',\;{\sf nop})$,
  with $I,I'\subseteq[m]$ being sets of registers
\item $({\sf true},\;{\sf set}\,i)$ or $({\sf true},\;{\sf nop})$
\end{itemize}

Consider a transition labeled by $({\sf eq}\,I\mathrel{\sf and}{\sf neq}\,I',{\sf nop})$.
It is encoded by transitions labeled $({\sf eq}\,i,{\sf nop})$ for all~$i$ such that $I\subseteq I_i$ and $I'\cap I_i=\emptyset$.
In other words, we enumerate all supersets of~$I$ that to not intersect~$I'$ and are among the sets of registers which hold the same value, according to the encoding of the source state.
If $I=\emptyset$, then a transition labeled $({\sf fresh},{\sf nop})$ must be added in the encoding.
Because the action is ${\sf nop}$, the partition $I_1$, \dots,~$I_m$ needs not be updated, so $I'_i=I_i$ for all~$i$.

Consider a transition labeled by $({\sf true},{\sf set}\,i)$.
Either the letter is in some registers or it is in none.
Thus, the encoding consists of transitions labeled $({\sf eq}\,j,{\sf nop})$ for all~$j\in[m]$ and transitions labeled $({\sf fresh},{\sf set}\,k)$~for a $k$.
The target~$k$ of the assignment is an index such that $I_k=\emptyset$ or, if there is no such $I_k$, an index such that $I_k=\{i\}$.
The partition $I_1$, \dots,~$I_m$ needs to be updated.
\[I'_j=
\begin{cases}
\{i\} &\text{if $j=k$}\\
I_j-\{i\} &\text{otherwise}
...
Consider a transition labeled by $({\sf true}, {\sf nop})$.
It is a special case of $({\sf eq}\,I\mathrel{\sf and}{\sf neq}\,I',{\sf nop})$, with empty $I$~and~$I'$.
...
They are handled by adding one register.
We omit the exact encoding.

We have proved the following result.

\begin{lemma}[TOPL to RA]\label{lemma:topl-to-ra}
There exists an algorithm that, given a TOPL automaton~${\cal A}$ over the alphabet~$V^n$, with $m$~registers, $|Q|$~states, and $|\delta|$~transitions, builds a register automaton~${\cal A}'$ over the alphabet~$V$, with $m+1$~registers, $O(n\,2^{m^2}|Q|)$~states, and $O(mn\,2^{m^2}|\delta|)$~transitions such that
