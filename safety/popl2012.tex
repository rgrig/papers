...
\item The containment, equivalence, and universality problems are decidable for $2$~registers.~\cite{dblp:conf/focs/kaminskif90}~\cite{dblp:conf/mfcs/nevensv01}.
...
property SingletonPattern
  start -> a:  \pattern{I} := \pattern{*}.getInstance()
  a -> error: \pattern{i}:=! \pattern{*}.getInstance()
\end{Verbatim}
\delimitVerbatim}
