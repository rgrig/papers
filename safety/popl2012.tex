...
\item \emph{Collect call stacks}.
  ...
  \item {\em Approximantion method.} The tool allow different ways in which the history informations are forgotten when we have reached the max amount of info we can keep.
\end{itemize}

\paragraph{The TOPL Compiler.} \label{sec:toplc} % <<<
The compiler has the following five phases.
\begin{enumerate}
...
info used by the TOPL property. Moreover it contains code that configures the checker.
%
...
It implements the execution of transition in  TOPL automata with rollback.
...
The set of active states of a nondeterministic automaton can be unbounded.
Therefore, like in the case of trace lengths, we impose a bound on the cardinality of set of active states. 
The bound can be configured by the user that needs also to decide 
the  strategy for forgetting states. We implemented three strategies: random, newest, and oldest.
Newest\slash oldest refer to a time attached to states.
The time of a state is the number of times a transition (i.e., {\sc STEP}) was taken in order to produce the state.
