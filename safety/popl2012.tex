...
\maketitle
\begin{abstract} % <<<
We introduce a new formal framework aimed at checking temporal safety properties of Java programs.
...
over time (typestate),  or expressing finitely properties that may require an unbounded number of different objects (they would need infinitely
long specifications).

%Systems for run time verification so far have been.... eiterh with this or that problem.
%- needs of infinite objects or lenghts of the property
%- talk only of one object
%- expressing trivial properties
...
automata: a high-level one where properties about programs are naturally expressed, and a low level one where the connection with  register
automata can be formally defined. Finally we defined a formal language TOPL (Temporal Object Property Language) mapping directly on the 
...
the programmer.  Thus our technique provides developers with a flexible methodology for enforcing the safety properties with which the code should comply. Currently, the only option for programmers is to insert explicit checks directly in the code.
Being parametric our system provides with the ability to tune the system in terms of 
coverage,  overhead, and trace reporting. 
...
% Say how we solve that and therefore we are better.
%It makes sense to try to define a formalism to overcome these shortcoming of existing systems.
%RA have certain interesting capabilities that could be useful.
%In this paper we define a runtime verfication technique which overcome some of the above problems.
%- It's based on formal systems (extension of RA), can reuse results (for example emptyness) to say if the property makes sense
%- It is easy for programmers to write. TOPL is tailored to OOP.
%
%We define a precise correspondence between RA and TOPL, therefore our running system lays on rigorous mathematical foundation.
...
\item We have implemented our parametric framework  in a tool that automatically check at runtime violations of TOPL properties in Java programs. A parameter $k$ is used to tune the precision of the system.
...
The guard requires the current letter to be different from the values in the set~$I'$ of registers of~${\cal A}_2$.
The current letter must be different from the values held in the set~$r(I')$ of registers of~${\cal A}_3$.
It may be either fresh, or equal to a register outside of the set~$r(I')$.
