...
of~${\cal A}$ is reached.
Here $k$,~$r$, $u_1$, \dots,~$u_m$ vary subject to the following constraining invariants:
\begin{itemize}
\item the stores of~${\cal A}$ are injective: $u_i \ne u_j$ if $i\ne j$
\item for $i\in[m_\rho]$, the value of register~$i$ of~${\cal A}_\rho$ is stored in ${\cal A}$'s registers: $ u_{r(i)} = v_i $
\item for $0\le k'<h$ and $\ell_{k'}=(v'_1,\ldots,v'_n)$ and $1\le i'\le n$, the $i'$th component~$v'_{i'}$ of letter~$\ell_{k'}$ is stored in ${\cal A}$'s registers: $u_{r(m_\rho+((k+k')\bmod(d-1))n+i')}=v'_{i'}$
\end{itemize}
All transitions of~${\cal A}$ have the form
\[\bigl((q_\rho,h,k,r),\;(g,a),\;(q'_\rho,h',k',r')\bigr)\]
Some correspond to transitions $(q_\rho,\bar\lambda,q'_\rho)$ of~${\cal A}_\rho$, and some have $q_\rho=q'_\rho$ and encode the skip behavior.
In all cases, $a$,~$k'$, and~$r'$ do not depend at all on the transitions of~${\cal A}_\rho$.
