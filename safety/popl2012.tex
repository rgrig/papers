...
A letter~$\ell$ is an $n$-tuple of values from a possibly infinite set~$V$;
...
Given a number~$m$ of registers, an arity~$n$ for letters, and a set~$V$ of values, the set $A=\Sigma\to(S\times S)$ of possible actions is infinite. Therefore we want to focus on a subset of $A$.
\dd{Ok, I changed it in this was. "rather big" didn't mean anything an seem to come out of the blue. Also I think you meant "countable"
rather then "uncountable" since the latter would be the cardinality of the real numbers. I'd be surprised if you meant that. To avoid problem I used the word infinite.}
...
\dd{should the transition with nop remain in 2? \rg{no}}
\end{example}
