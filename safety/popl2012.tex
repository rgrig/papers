...
The tradeoff between the coverage and the overhead of the monitoring system is tunable by means of few parameters.
...
the approach and allow us to reuse many of the results for register automata on TOPL automata. For example, given a TOPL property, we can decide whether its language is empty.  The formal development is completely hidden from 
the programmer.  
%Thus TOPL provides developers with a flexible methodology for enforcing safety constraints on their code.
...
property:
\eqquote{Any value introduced by the {\tt input()} method should not reach the  {\tt sink()} method without passing first from the {\tt sanitizer()} method.}{q:concur-it2}
This property is at first sight simple. However, it's difficulty can vary depending from the context. Consider in fact the case where
...
The number of user inputs, and therefore of concatenations, it is not known a priori and in general unbounded.
Consequently we will have a possibly an unbounded number of tainted objects. 
...
The problem of rebinding of variable in the specification arises also in other contexts. For example, 
when  reasoning about the evolving shape of dynamic allocated data-structures. 
...
\end{verbatim}
If the list {\tt l} is circular or pan-handle shape the loop will diverge.
...
\emquote{The shape of the list should not be neither circular nor pan-handle.}
...
value of the specification variables.  This is because the way to encode the property is by checking, when using the list, that we reach some element we have seen before. 
...
The length of a transition is the number of labels it has.
Since the set~$\delta$ of transitions is finite, there is a maximum~$d$ of the transition lengths.
The rollback configuration $((q,s),w)$ is simulated by some configuration $((q,s),w')$, where $w'$ is a prefix of~$w$, and $|w'|<d$.
Because $|w'|$ is bounded, we can represent $((q,s),w')$ by a simple configuration $(q',s')$ as follows.
The store~$s'$ contains the values from~$s$ and those from~$w'$.
If $s$~has $m$~registers, and letters are $n$-tuples, then $s'$~has $m+(d-1)n$ registers.
The state~$q'$ encodes both the state~$q$ and which of the registers store~$w'$.
\rg{TODO:Make the above more informal.}

\begin{lemma}[rTOPL to TOPL]\label{lemma:rtopl-to-topl}
There exists bla
\end{lemma}
\begin{proof}
\rg{TODO: The formal version of the proof.}
\end{proof}
...
injection. There is, however, in the interest of user-friendlyness,
...
with parameter values and a time-tag. The time-tag is either $call$ or $ret$,
corresponding to observing the method being called or returned
from. If the time-tag is $ret$, the event also carries the return value.
...
call\ m \mid return\ m
\]
where $m$ is one of the method names mentioned in $P$.
...
We are only concerned with the subset $E \times V^{n+1}$, ie the tuples where the first component is an event
id. If $P$ has a maximal arity of 5, the event $call\ m(a, b, c)$
would be understood as $(call\ m,  a, b, c, \bot, \bot, \bot)$ by
...
value. The event $ret\ r = m(a, b, c)$
would be understood as $(ret\ m,  a, b, c, \bot, \bot, r)$ by
$A_P$.
...
Further, every element of $V_L$ acts as a pattern that matches only the value it denotes,
...
A label is of one of two forms:
\[
  e ::=
  call\;m(x_1, \ldots, x_n) \mid
  ret\;x := m(x_1, \ldots, x_n)
\]
where $x$ and $x_1, \ldots, x_n$ are all patterns.

In order to match elements from $V_L$, there is an extra register in
$A_P$ for each unique element mentioned by $P$. This register contains
that value in the initial state of $A_P$ and is never overwritten. For
each pattern $x$ which is not *, we define two functions $reg(x)$ and
$pred(x)$. $reg(x)$ gives the associated register and $pred(x)$ gives
the corresponding predicate, i.e.
\[
\begin{array}{r c l}
  pred(\mathtt{v})(j) &=& {\sf eq}\,reg(\mathtt{v})=j \\
  pred(\mathtt{!v})(j) &=& {\sf neq}\,reg(\mathtt{v})=j \\
  pred(l)(j) &=& {\sf eq}\,reg(l)=j \\
  pred(*)(j) &=& {\sf true} \\
\end{array}
\]
...
\[
\begin{array}{r c l}
  \den{call\;m(x_1, \ldots, x_n)} &=& \den{m(x_1, \ldots, x_n)} \\
  \den{ret\;x := m(x_1, \ldots, x_n)} &=& {\sf eq}\,r(x) = n+1 \land \den{m(x_1, \ldots, x_n)} \\
  \den{m(x_1, \ldots, x_n)} &=& pred(x_1)(1) \land\ldots\land pred(x_n)(n) \\
\end{array}
\]

\subsection{Atomic call/return events}
\rlp{This is why we need rollback}

\subsection{Directives}
\rlp{Describe the directives briefly}
...
%It is this file that must be recompiled if one wishes to change the parameters of the checker. 
