...
There exists an algorithm that, given a rollback TOPL automaton~${\cal A}$ over the alphabet~$V^n$ with $m$~registers, $|Q|$~states, and $|\delta|$~transitions of length~$\le d$, builds a simple TOPL automaton~${\cal A}'$ with $m'=m+(d-1)n$ registers, $O(d^2m'^{m'}|Q|)$ states, and $x$~transitions such that
\[ w\in{\cal L}_\rho({\cal A}) \iff w\underbrace{\ell_\bot\ldots\ell_\bot}_{\text{$d$~times}}\in{\cal L}({\cal A}')\]
...
The first two forms describe observable events and will be ranslated
...
usefulness, two directives were added to the language. A prefix
directive for convenience when specifying method names and an
observing directive for pre-filtering events.
...
\paragraph{Iterators.} 
\autoref{fig:first.topl} (top)  formalise the temporal property~\eqref{eq:} from Section~\ref{sec:}
...
The following is a TOPL encoding of the property~\eqref{} in Section~\ref{sec:}.
