...
Given a TOPL Property $P$, we wish to construct a TOPL rollback
automaton $A_P$,
such that the language of the automaton is the set of event traces of $P$.
Thus we choose the values of $A_P$ to be the observable events
of $P$.
...
Thus, for $A_P$, the values of interest come from three
sources; event ids and Java values: The set $E$ of event ids is given by
grammar
\[
call\ m \mid return\ m \mid m
\]
where $m$ is one of the method names known to $P$, ie method
names mentioned on guards of $P$ prefixed with strings mentioned in
$P$'s prefix directive.

The set $V_J$ of primitive values is the set of Java values
...
The values are then $V = E + V_J $, and the alphabet is $V^{n+1}$,
where $n$ is the maximal arity of a method mentioned in the property.

We are only concerned with the subset $E \times V^n$, ie the tuples where the first component is an event
id. If $P$ has a maximal arity of
5, the event $call\ m(a, b, c)$  would be understood as $(call\ m, a,
b, c, \bot, \bot)$ by $A_P$.

We now explain how the labels of $P$ are transformed into guards and
actions of $A_P$.
