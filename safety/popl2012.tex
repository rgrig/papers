...
We do so by replacing the distribution function~$r$ by one that is undefined at $\iota(k,i')$ for $i'\in[n]$, by decrementing~$h$, and by incrementing~$k$.
...
%The store maps property variables ({\tt in}) to values ({\tt Object}).
