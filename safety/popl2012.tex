...
Given a number~$m$ of registers, an arity~$n$ for letters, and a countable set~$V$ of values, the set $A=\Sigma\to(S\times S)$ of possible actions is uncountable.
...
{\cal L}_\rho({\cal A})=\\ 
  \{\,w_1\ldots w_k\mid
    ...
    $y_k$~final}\,\}
\end{multline*}
A simple automaton~${\cal A}$ has both a simple language~${\cal L}({\cal A})$ and a rollback language~${\cal L}_\rho({\cal A})$.
The languages ${\cal L}({\cal A})$ and ${\cal L}_\rho({\cal A})$ are usually distinct.
...
So, we synthesize a guard~$g$ that holds exactly in is situation.
...
However, we can negate the simple guard ${\sf eq}$ and ${\sf neq}$, we can use distributivity to put $g$ in disjunctive normal form, and we can simulate disjunction by parallel transitions.
