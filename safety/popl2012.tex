...
We write a number of properties expressing correct use of the
JDK. The entire DaCapo suite is instrumented with these. Further,
for three selected projects we write a few TOPL properties that encode informal temporal information that exists in the code comments.

We measure both time and space overhead. It turns out that space
overhead is negligible, so we only report time overhead. For the JDK
properties, it can be
found in~\autoref{table:experiments} for various configurations of the
checker, and  for the original bytecode, with no instrumentation. For
the individual project properties it can be found in~\autoref{table:overhead}.
%
