...
TOPL automata are a variant of {\it register automata\/}~\cite{dblp:journals/tcs/kaminskif94}, which were introduced in~1994.
...
This work validates the intuition by exhibiting a working runtime verification system.
On the other hand, automata theorists are mainly interested in intrinsic qualities of the model.
For example the emptiness problem is NP-complete~\cite{dblp:journals/tcs/sakamotoi00} or PSPACE-complete~\cite{dblp:journals/tocl/demril09}, depending on the exact variant of register automata.
Such results suggest techniques to statically analyze and transform TOPL properties -- an opportunity for future work.
...
{\it JavaMOP\/}~\cite{dblp:journals/sttt/meredithjgcr12} is a framework for runtime verification of Java programs.
Roughly, the alphabet considered is $C\times V^n$, where $C$~is finite and $V$~is infinite.
Conceptually, the framework implements a function $(C\times V^n)^*\to {\cal P}(C^*)$ that slices a trace over an infinite alphabet into several traces over a finite alphabet.
Then one can use any of several plugins that act as recognizers $C^*\to\B$ of languages over a finite alphabet.
The slicing function is fixed.
The plugins, on the other hand, are arbitrary functions as far as the framework is concerned.
They are not even constrained to be computable.
In practice, however, they implement monitors for various decidable logics, such as finite state machines or LTL\null.
The formal treatment~\cite{dblp:journals/corr/abs-1112-5761} claims that results hold for $n=|\N|$, but the proofs and the implementation handle only the case $n\in\N$.
Experimental results show that JavaMOP is the most efficient runtime verification system~\cite{dblp:journals/sttt/meredithjgcr12}.

There exist TOPL properties that are not expressible in JavaMOP with {\em any} existing or potential plugin.
For example taint properties that allow deriving new dirty values from old dirty values.
Consider a taint property that identifies a source of dirty strings, a sink, and says that the result of concatenating a dirty string with another string is dirty itself.
Intuitively, a fixed $n\in\N$ does not suffice because the number of dirty strings derived from one source is unbounded.
(In the jargon of JavaMOP this property requires an unbounded number of parameters.)
