...
% plan
%   - intro: see if it works, see the overhead
%   - methodology: dacapo, 6 projects, 6 properties, measure overhead, no bugs
%   - a bug's death: story of bug in tomcat and in dacapo
...
% plan
% - expect no bugs: we assume these projects are well-tested (e.g., many users)
% - dacapo
% - 6 projects actively developed and used
% - grep comments, check code,  write property
% - measure overhead for various parameters
% - computer specs
...
We measure the overhead on the test suite DaCapo-9.12~\cite{dblp:conf/oopsla/dacapo}.
...
\rg{The versions are: Eclipse 3.5.1, TODO.}
...
\multicolumn{3}{c}{\multirow{2}{*}{DaCapo benchmark}}
  & \multicolumn{10}{c}{Number of monitored configurations} \\ \cmidrule{4-13}
&&
  &\multicolumn{2}{c}{$\le0$}
...
\multirow{2}{*}{tomcat}
  ...
  & $5.6\rm\,s$ & $\pm0.2\rm\,s$
  & $9.0\rm\,s$ & $\pm0.3\rm\,s$
  & $43.5\rm\,s$ & $\pm1.2\rm\,s$
  & $43.9\rm\,s$ & $\pm1.0\rm\,s$ \\
  &&
  & \multicolumn{2}{c}{$\sim12\rm\,MiB$}
  & \multicolumn{2}{c}{$\sim13\rm\,MiB$}
  & \multicolumn{2}{c}{$\sim37\rm\,MiB$}
  & \multicolumn{2}{c}{$\sim14\rm\,MiB$}
  & \multicolumn{2}{c}{$\sim13\rm\,MiB$}
  \\[1ex]
\multirow{2}{*}{pmd}
  & $5.2\rm\,s$ & $\pm0.4\rm\,s$
  & $5.4\rm\,s$ & $\pm0.2\rm\,s$
  & $12.2\rm\,s$ & $\pm0.3\rm\,s$
  & $47.7\rm\,s$ & $\pm10.7\rm\,s$
  & \multicolumn{4}{c}{\multirow{2}{*}{time $>5.2\times100\rm\,s$}}\\
  &&
  & \multicolumn{2}{c}{$\sim?\rm\,MiB$}
  & \multicolumn{2}{c}{$\sim?\rm\,MiB$}
  & \multicolumn{2}{c}{$\sim?\rm\,MiB$}
  \\[1ex]
\multirow{2}{*}{h2}
  & $6.6\rm\,s$ & $\pm0.2\rm\,s$
  & $9.5\rm\,s$ & $\pm0.2\rm\,s$
  & $130.1\rm\,s$ & $\pm12.2\rm\,s$
  & \multicolumn{6}{c}{\multirow{2}{*}{time $>6.6\times100\rm\,s$}} \\
  &&
  & \multicolumn{2}{c}{$\sim?\rm\,MiB$}
  & \multicolumn{2}{c}{$\sim?\rm\,MiB$}
  \\
...
Heap sizes are rough estimates, obtained by sampling.
}
\end{table*}
...
and have it expanded to is the following:
\begin{align*}
...
The instrumented classes have the same behaviour as the original classes.
...
An \emph{observable} method is one whose name matches a pattern that appears in an \textsf{observing} directive.\dinocomment{observing directive has not been introduced before}
...
For instrumenting Java bytecode we use a fork of the library Barista~\cite{barista}.
