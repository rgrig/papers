...
For measuring the memory overhead we estimate the size of the heap every time the execution enters the TOPL checker, and take report the maximum.
Both the run time and the heap size are measured for varying settings of the TOPL checker: maximum number of active states (see later) to track, maximum history length, with and without collection of call-stack traces.
