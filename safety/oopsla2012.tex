...
This technique give rises to a flexible methodology to deal with the necessary safety properties that the programmer need to enforce to his code. 
...
This description serves as a rigorous documentation of which safety properties are enforced in the code.
...
For example, during testing they could allow for a very high overhead, whereas and they could chose to deploy the bytecode without any inserted check.
...
\end{figure} % >>>
\autoref{fig:first.steps} shows an execution of the program in \autoref{fig:first.java} and of an automaton for the property \texttt{InvalidateOtherIterators}.
...
step.
\rlp{Consider mentioning the steps in \autoref{fig:first.steps}.}
\paragraph{Step~1.}
...
\label{sec:syntax}
\dinocomment{maybe we should put back the syntax? Otherwise change the name of the section}
