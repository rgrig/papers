...
\conferenceinfo{FOOL'11}{October 23, 2011, Portland, Oregon, USA}
\copyrightyear{2011}
% TODO \copyrightdata{}
...
\begin{tikzpicture}
  ...
  \draw[transition,gray] (start) .. controls +(30:1cm) and +(-30:1cm) .. node[right]{$*$} (start);
...
property IteratorComodification
  prefix <java.util.\{Collection,Iterator\}>
  \textcolor{gray}{start -> start:* // implicit by convention}
...
\end{Verbatim}
\caption{A first example: TOPL property}
...
Labels look like method calls, there is a shorthand notation for parallel transitions, there is an implied loop on the vertex \start, the \texttt{prefix} directive offers some extra convenience, and so on.
...
A property has a name, a set of \texttt{prefix} directives, a set of \texttt{observing} directives, and a set of transitions.
...
Transitions may be tagged with \texttt{call} or \texttt{return} to specify exactly at what time they should be performed (see \autoref{sec:semantics.itopl} for details).
...
For each automaton variable \textit{var} there are two associated patterns.
The uppercase pattern \Verb@Var@ matches any value and writes it in the automaton variable \textit{var}.
The lowercase pattern \Verb@var@ reads the value of the automaton variable \textit{var} and only matches that value.
A Java literal surrounded by angle brackets acts as a pattern that matches only the value it denotes.
A wildcard~\Verb@*@ pattern matches any value.

A TOPL property is \emph{well-formed} when it satisfies the following conditions.
\begin{itemize}
\item For each automaton variable, a transition may prescribe at most one write.
In other words, a given uppercase pattern may appear at most once within a label.
\item Automaton variables must be written before being read.
In other words, lowercase patterns must be preceded by corresponding uppercase patterns on all paths coming from \start.
\end{itemize}
...
Each transition is labeled by a list of steps.
Each step has guards and actions.
There are several types of guards.
A tag guard may restrict the type of events (\texttt{call} or \texttt{return}).
A method guard requires that the method name and its arity match certain patterns.
Value guards impose restrictions on the values carried by events.
Both value guards and actions refer to the values carried by events using an index.

\begin{figure}
\grammar{
  Property& \b{property} Identifier Transition\* \cr
  Transition& Arc \b: TransitionStep\* \cr
  TransitionStep& \b(TagGuard MethodGuard ValueGuard\* Action\*\b) \cr
  TagGuard& \b* \| \b{call} \| \b{return} \cr
  MethodGuard& NamePattern \b[ IntegerPattern \b] \cr
  ValueGuard& \b[ ValueIndex \b] \b= \( Identifier \| Literal \) \cr
  Action& Identifier \b{:=} \b[ ValueIndex \b] \cr
...
% TODO: in implementation one is concrete, other is AST
...
The authors thank the anonymous reviewers for the thorough feedback.
