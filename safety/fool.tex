...
The first example (\autoref{sec:examples.steps}) is a fairly subtle property imposed on the users of the Java~API\null.
...
The following examples (\autoref{sec:examples.others}) illustrate the expressivity of the property language.

\subsection{Invalidating Iterators} \label{sec:examples.steps} % <<<
...
\end{figure}
\begin{figure}
\rg{TODO: Fix typesetting of patterns.}
...
\rg{TODO: Explain a bit more up-front the automaton variables $i$,~$j$ and~$c$, and what do they mean.}
\rg{TODO: Explain the general form of a label and what it is supposed to do, perhaps after the first step.}
...
Automata have a set of  {\em automata variables}\footnote{To avoid confusion we write  pattern  in underlined monotype and automata variable in italic.} which live in a different name-space from program variables.
By convention, patterns starting with an uppercase letter match any \emph{value}.
In this case, pattern  $\pattern{I}$~and~$\pattern{C}$ match the values held by the program variables {\tt i}~and~{\tt c}.
At this point, all conditions are met to perform the transition from \textit{start} to \textit{gotOne}.
Performing a transition involves storing the values on which the uppercase patterns matched into {\em automata variables}.
By convention, an uppercase pattern writes always in the automaton variable with the same name.
...
% >>>
\subsection{Other Examples} \label{sec:examples.others} % <<<
...
\par\medskip\noindent
\begin{Verbatim}
property BadIterator
  start -> s2: I := C.iterator()
  s2 -> error: i.next()
  s2 -> s4: true := i.hasNext()
  s2 -> s3: c.add(*), c.remove(*), ...
  s4 -> s2: i.next()
  s4 -> s3: c.add(*), c.remove(*), ...
  s3 -> error: call i.*[*]
\end{Verbatim}
...
\par\medskip\noindent
\begin{Verbatim}
property ConsecutiveNexts
  start -> one: I.next()
  one -> ok: true := i.hasNext()
  one -> error: i.next()
\end{Verbatim}
