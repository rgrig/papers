...
TOPL is designed to express properties over traces of events of the
following two types:
\[
\begin{array}{l}
  \mathtt{call}_{m[n]}\ [c, \this, v_1, \ldots, v_n] \qquad \mathrm{and} \\
  \mathtt{return}_{m[n]}\ [c, v]
\end{array}
\]
The call events are matched by labels of the form
\[
  <c>\ \mathtt{call}\ m(p_1, \ldots, p_n)
\]
...
The guard is then the conjuntion of all these guard terms together
with the context guard $c = \this$.
...
% There are two main aspects of the desugaring from TOPL to iTOPL.
% The translation $\desugar$ from TOPL to iTOPL is defined as follows:
% \rlp{This should probably be a figure.}
% \[
% \begin{array}{r @{\,} c @{\,} l}
%   \desugar(\mathtt{property}\ id\ us\ ts)
%   & = &
%   \mathtt{property}\ id\ (map\ \desugar_{trans}\ ts) \\
%   \desugar_{trans}(arc : l_1\ldots l_n)
%   & = &
%   arc : \desugar_{lab}\ l_1\ldots arc : \desugar_{lab}\ l_n \\
%   \desugar_{lab} (<c>\ t\ mp)
%   & = &
%   [0] = c \land \desugar_{lab}(t\ mp) \\
%   \desugar_{lab} (\mathtt{call}\ r.m(args))
%   & = &
%   (\mathit{guard}_{call}(m, r::args), \\ && \phantom{(}
%   \mathit{action}(r::args)) \\
%   \desugar_{lab} (\mathtt{return}\ r := m[n])
%   & = &
%   (\mathit{guard}_{return}(m, r, n), \\ && \phantom{(}
%   \mathit{action}([r])) \\
%  \desugar_{lab} (l := r.m(args))
%   & = &
%   [
%   (\mathit{guard}_{call}(m, r::args), \\ && \phantom{[(}
%   \mathit{action}(r::args)), \\ && \phantom{[}
%   (\mathit{guard}_{return}(m, l, |args|), \\ && \phantom{[(}
%   \mathit{action}([l]))
%   ]
% \end{array}
% \]
