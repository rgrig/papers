...
The long term aim of our project is the automatic verification of safety temporal properties for Java programs of realistic size.
...
\end{itemize}
This paper addresses the first point by introducing \TPL\ (\textbf{t}emporal \textbf{o}bject-oriented \textbf{p}roperty language, pronounced like `topple').
...
\end{quote}
\noindent
It is apparent that the formalization of the above constraint needs to keep track of the relation of several objects (at least two iterators, and one collection) and their interaction.
...
\item in the case of the Java API, does not provide a trace of the relevant events that lead to an error.
\end{itemize}
...
pertaining to method calls. A method call $x.m(v_1, \ldots, v_n)$ will
generate the 
following two events:
\[
\begin{array}{l}
  \mathtt{call}_{m[n]}\ [x, v_1, \ldots, v_n] \qquad \mathrm{and} \\
  \mathtt{return}_{m[n]}\ [x, v]
\end{array}
\]
The call events are matched by labels of the form
\[
  \mathtt{call}\ m(p_1, \ldots, p_n)
\]
...
should have the approrpiate values. That is all lowercase automaton
variables $x = p_i$ amongst the value
patterns give rise to a guard term of the form
\[
event[i+2] = x
\]
(or $event[i] \neq x$ in the case of $p_i = !x$), referring to the
array of values carried by the event.
The guard is then the conjuntion of all these guard terms.

The action expresses that automaton variables should be updated. So
all uppercase automaton variables $X = p_i$ amongst the value
...
arguments matching $p_1, \ldots, p_n$ and a return with the value
matching (or updating) $p$.
In this case, the iTOPL label is a list of (guard, action)-pairs of
length 2. The first element corresponds to the label $\mathtt{call}
m(p_1, \ldots, p_n)$ and the second to the label $\mathtt{return} m[p]$.
...
In~\cite{disney2011} contratcs are used to express legal traces of
programs in a functional language with references. This is where we
found the atomicity example. In this setting, however, the granularity
of events is implicit
