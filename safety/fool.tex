...
\autoref{fig:first.steps} shows an execution of the program and of a \emph{nondeterministic} automaton for the property \texttt{IteratorComodification}.
...
At vertex \texttt{one} the variables $x$~and~$c$ are initialized, and the variable~$c$ is not initialized;
...
We say \emph{uppercase pattern} for an automaton variable pattern that starts with an uppercase letter;
we say \emph{lowercase pattern} for an automaton variable pattern that starts with a lowercase letter.
\end{notation}
...
One can write the TOPL property without understanding some implementation of Java's collections library.
...
In this case, all method calls within \texttt{SomeClass} are observed.
...
\grammar{
  ...
  Using& \b{prefix} \b< StringPattern \b> \cr
...
In both cases the TOPL labels give rise to an iTOPL label with one (guard, action) pair.
