...
\begin{abstract} % <<<
  In this paper we present a new specification language for temporal
  safety properties aimed at object-oriented languages.  
  The language is expressive enough to represent relationships between objects and it is designed with the goal of performing dynamic/static analysis of object-oriented software.
\end{abstract}
...
Many safety properties can be expressed using the framework of {\em typestate}~\cite{Strom and Yemini 1986} which uses finite state automata for specifying conformance or violation w.r.t. a temporal safety property.
...
It has the following carachteristics:
\begin{itemize}
\item It is designed for OOP. 
And since object-oriented languages like Java make heavy use of the heap, we have designed our language based on separation logic~\cite{reynolds02} a formalist known to be effective and concise in specifying heap related properties.
...
Finally, Section~\ref{sec:conclusion} conclude the paper.
% >>>
\section{Example} % <<<
