...
locks should be acquired before the memory they protect is accessed and should eventually be released.
...
\end{Verbatim}
\delimitVerbatim
The method \Verb@makeResource@ could also be a constructor.
In simple cases, resources cannot repeatedly move between the \Verb@released@ and \Verb@acquired@ state.
\delimitVerbatim
\begin{Verbatim}
property ResourceSafetySimple
  start -> created:    R := makeResource()
  created -> disposed: r.release()
  disposed -> error:   r.use()
\end{Verbatim}
\delimitVerbatim
...
We model stores as partial functions with finite domain (sometimes
called dictionaries):
\[
...
carries not only a single guard but potentially a list of guards. We
call the lenght of this list, the \emph{depth} of the guard.
...
\begin{array}{r c l}
  s_0 & = & s \\
  s_i & = & a_i(e_{i-1}, s_{i-1})
\end{array}
\]
If the transition is enabled and taken, the store for the vertex at
the end of the arc will be $s_n$.
...
dropping event s is not allowed if there are any enabled
...
\[
\{ e \mid
\exists\sigma'e',\;((\mathtt{error},\sigma'),e')\in\mathit{NdetStep}^\star\;\{((\mathtt{start},0),e)\}
\}
\]
